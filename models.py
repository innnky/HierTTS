...
class FFTBlock(torch.nn.Module):
    ...
    def __init__(self, d_model, d_inner,
        ...
        self.pos_ffn = PositionwiseFeedForward(
            d_model, d_inner, dropout=dropout, fft_conv1d_kernel_size=fft_conv1d_kernel_size, causal=causal,
            deconline=deconline)

    def forward(self, enc_input, non_pad_mask=None, slf_attn_mask=None, mems=None, cache=None, bias0=0, bias1=0,
                lookahead=0):
        if mems is not None:
...
class FFTmodule(nn.Module):
    ...
    def __init__(self, config, input_dim, decoder_layer, decoder_hidden, decoder_head, fft_conv1d_filter_size,
                 fft_conv1d_kernel_size, dropout, max_seq_len, deconline=False):
        ...
        self.layer_stack = nn.ModuleList([FFTBlock(
            self.d_model, self.d_inner, self.n_head, self.d_k, self.d_v, fft_conv1d_kernel_size=fft_conv1d_kernel_size,
            dropout=dropout, deconline=deconline) for _ in range(decoder_layer)])
        ...
        if self.deconline:
            ...
            for i in range(math.ceil((max_len - firstpackage) / self.frames_current) + int(firstpackage != 0)):
                if i == 0 and firstpackage != 0:
                    ...
                    cur_output = enc_seq[:, (i - int(firstpackage != 0)) * self.frames_current + firstpackage:(
                                                                                                                          i + 1 - int(
                                                                                                                      firstpackage != 0)) * self.frames_current + self.frames_overlap + firstpackage,
                                 :]
                    cur_enc_pos = enc_pos[:, (i - int(firstpackage != 0)) * self.frames_current + firstpackage:(
                                                                                                                           i + 1 - int(
                                                                                                                       firstpackage != 0)) * self.frames_current + self.frames_overlap + firstpackage]
    ...
    def inference(self, cur_enc_seq, cur_enc_pos, return_attns=False, mems=None, current_length=None, caches=None,
                  frame_beg=None):
        dec_slf_attn_list = []
        ...
        if self.deconline:
            ...
            for idx in range(len(self.layer_stack)):
                ...
                if frame_beg is not None:
                    ...
                    mems_idx = mems[idx] if mems is not None else None
                cache_idx = caches[idx].transpose(1, 2) if mems is not None else torch.zeros(
                    (dec_output.size(0), cache_start, dec_output.size(2))).to(device).transpose(1, 2)
                bias = mems_idx.size(1) if mems_idx is not None else 0
...
class AdaptFFT(nn.Module):
    def __init__(self, config, input_dim, decoder_layer, decoder_hidden, decoder_head, condition_dim=0, deconline=False,
                 fft=False, outdim=None) -> None:
        super().__init__()
        self.fft = fft
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(self,
        ...
        self.phn_enc = AttentionRNN(embed_size=config.encoder_hidden, gru_hidden=32, bidirectional=True,
                                    pooling_mode=self.pooling, linear=True)
        self.word_enc = AttentionRNN(embed_size=128, gru_hidden=128, bidirectional=True, pooling_mode=self.pooling,
                                     linear=True)
        self.sent_enc = AttentionRNN(embed_size=128, gru_hidden=128, bidirectional=True, pooling_mode=self.pooling,
                                     linear=True)
        self.length_regulator = LengthRegulator(312, config.max_seq_len)

        # wav2frame
        self.wav2frame = PosteriorEncoder(spec_channels, inter_channels.frame, hidden_channels, 5, 1, 16,
                                          gin_channels=gin_channels)
        ...
        self.phn2frame = AdaptFFT(config, inter_channels.phn, config.decoder_layer.phn2frame,
                                  config.decoder_hidden.phn2frame, config.decoder_head.phn2frame,
                                  deconline=config.deconline, fft=True)
        self.phn2frame_comb = DecCombinerCell(config.encoder_hidden, inter_channels.phn, inter_channels.phn,
                                              self.dec_comb) if self.straight_phn else DecCombinerCell(
            config.decoder_hidden.subword2phn, inter_channels.phn, inter_channels.phn, self.dec_comb)
        self.phn2frame_proj = GaussProj(config.decoder_hidden.phn2frame, inter_channels.frame)
        self.out = nn.Linear(256, 80)

        # phn2subword
        self.phn2subword = AttentionRNN(embed_size=256 + config.encoder_hidden + 1, gru_hidden=128, bidirectional=True,
                                        pooling_mode=self.pooling) if self.txt_cond else AttentionRNN(embed_size=256,
                                                                                                      gru_hidden=128,
                                                                                                      bidirectional=True,
        ...
        self.subword2phn = AdaptFFT(config, inter_channels.subword, config.decoder_layer.subword2phn,
                                    config.decoder_hidden.subword2phn, config.decoder_head.subword2phn,
                                    condition_dim=config.encoder_hidden, fft=True)
        ...
        # subword2word
        self.subword2word = AttentionRNN(embed_size=256 + 128, gru_hidden=128, bidirectional=True,
                                         pooling_mode=self.pooling) if self.txt_cond else AttentionRNN(embed_size=256,
                                                                                                       gru_hidden=128,
                                                                                                       bidirectional=True,
                                                                                                       pooling_mode=self.pooling)
        ...
        self.word2subword = AdaptFFT(config, inter_channels.word, config.decoder_layer.word2subword,
                                     config.decoder_hidden.word2subword, config.decoder_head.word2subword,
                                     condition_dim=128, fft=True)
        self.word2subword_comb = DecCombinerCell(config.decoder_hidden.sent2word, inter_channels.word,
                                                 inter_channels.word, self.dec_comb)
        self.word2subword_proj = GaussProj(config.decoder_hidden.word2subword, inter_channels.subword)

        # word2sent
        self.word2sent = AttentionRNN(embed_size=256 + 128, gru_hidden=128, bidirectional=True,
                                      pooling_mode=self.pooling) if self.txt_cond else AttentionRNN(embed_size=256,
                                                                                                    gru_hidden=128,
                                                                                                    bidirectional=True,
                                                                                                    pooling_mode=self.pooling)
        ...
        self.sent2word = AdaptFFT(config, inter_channels.sent, config.decoder_layer.sent2word,
                                  config.decoder_hidden.sent2word, config.decoder_head.sent2word, condition_dim=128,
                                  fft=True)
    ...
    def forward(self, phn, phn_lengths, y, y_lengths, txt, txt_lengths, txt2sub, sid=None, phn2frame_ali_m=None,
                phn2frame_ali=None, subword2phn_ali_m=None, subword2phn_ali=None, word2subword_ali_m=None,
                word2subword_ali=None, sent2word_ali_m=None, sent2word_ali=None, word_lengths=None, sub2sub=None,
                subword_lengths=None, log_D=None, step=None):
        ...
        subword_mask = torch.unsqueeze(commons.sequence_mask(subword_lengths, torch.max(subword_lengths)), 1).to(
            y.dtype)
        ...
        subword_reps = self.phn2subword(
            torch.cat([phn_reps, phn_embeded, torch.sum((~phn2frame_ali_m).float(), 2).unsqueeze(1)], 1),
            subword2phn_ali_m) if self.txt_cond else self.phn2subword(phn_reps,
                                                                      subword2phn_ali_m)  # subword_ali unnique
        ...
        posterior_sent = D.Normal(m_sent_p + m_sent_res, std_sent_p * std_sent_res) if False else D.Normal(m_sent_res,
                                                                                                           std_sent_res)
        ...
        posterior_word = D.Normal(m_word_p + m_word_res, std_word_p * std_word_res) if self.residual else D.Normal(
            m_word_res, std_word_res)
        ...
        posterior_subword = D.Normal(m_subword_p + m_subword_res,
                                     std_subword_p * std_subword_res) if self.residual else D.Normal(m_subword_res,
                                                                                                     std_subword_res)
        ...
        posterior_phn = D.Normal(m_phn_p + m_phn_res, std_phn_p * std_phn_res) if self.residual else D.Normal(m_phn_res,
                                                                                                              std_phn_res)
        ...
        posterior_frame = D.Normal(m_frame_p + m_frame_res, std_frame_p * std_frame_res) if self.residual else D.Normal(
            m_frame_res, std_frame_res)
    ...
    def emb_infer(self, phn, phn_lengths, y, y_lengths, txt, txt_lengths, txt2sub, sid=None, phn2frame_ali=None,
                  subword2phn_ali=None, word2subword_ali=None, word_lengths=None, sub2sub=None, subword_lengths=None,
                  log_D=None):
        # frame_mask = torch.unsqueeze(commons.sequence_mask(y_lengths, y.size(2)), 1).to(y.dtype)
        phn_mask = torch.unsqueeze(commons.sequence_mask(phn_lengths, phn.size(1)), 1).to(y.dtype)
        subword_mask = torch.unsqueeze(commons.sequence_mask(subword_lengths, torch.max(subword_lengths)), 1).to(
        ...
        subword_reps = self.phn2subword(torch.cat([phn_reps, phn_embeded, phn2frame_ali.unsqueeze(1)], 1),
                                        subword2phn_ali) if self.txt_cond else self.phn2subword(phn_reps,
                                                                                                subword2phn_ali)  # subword_ali unnique
    ...
    def infer(self, phn, phn_lengths, txt, txt_lengths, txt2sub, sid=None, phn2frame_ali_m=None, phn2frame_ali=None,
              subword2phn_ali_m=None, subword2phn_ali=None, word2subword_ali_m=None, word2subword_ali=None,
              sent2word_ali_m=None, sent2word_ali=None, word_lengths=None, sub2sub=None, subword_lengths=None):
        ...
        if temperature['sent'] != 0:
            prior_sent = D.Normal(m_sent_p, std_sent_p * temperature[
                'sent'])  # if self.zero_txt else D.Normal(m_sent_p, std_sent_p)
    ...
    def analysis(self, phn, phn_lengths, txt, txt_lengths, txt2sub, sid=None, phn2frame_ali=None, subword2phn_ali=None,
                 word2subword_ali=None, word_lengths=None, sub2sub=None, subword_lengths=None):
        phn_mask = torch.unsqueeze(commons.sequence_mask(phn_lengths, torch.max(phn_lengths)), 1).to(phn.dtype)
        subword_mask = torch.unsqueeze(commons.sequence_mask(subword_lengths, torch.max(subword_lengths)), 1).to(
            phn.dtype)
        sent2word_ali = word_lengths.unsqueeze(1)  # 改
        ...
        for key in z_phn_dict.keys():
            s = self.phn2frame_comb(phn_embeded, z_phn_dict[key]) if self.straight_phn else self.phn2frame_comb(
                s_phn_dict[key], z_phn_dict[key])
