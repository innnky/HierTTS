...
def run(rank, n_gpus, hps):
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(rank, epoch, hps, [net_g, net_d, stft_criterion], [optim_g, optim_d],
                               [scheduler_g, scheduler_d], scaler, [train_loader, eval_loader], logger,
                               [writer, writer_eval])
        else:
            train_and_evaluate(rank, epoch, hps, [net_g, net_d, stft_criterion], [optim_g, optim_d],
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    txt2sub, sub2phn_m, sub2phn_e, txt, txt_len, word2sub_m, word2sub_e, word_len, sub2sub, sub_len, names, sent2word_m,
    sent2word_e) in enumerate(train_loader):
        ...
        with autocast(enabled=hps.train.fp16_run):
            y_hat, ids_slice, kl_sent, kl_word, kl_subword, kl_phn, kl_frame, D_loss = \
                net_g.forward(x, x_lengths, spec, spec_lengths, txt, txt_lengths, txt2sub, speakers, d_target_m,
                              d_target_e, sub2phn_m, sub2phn_e, word2sub_m, word2sub_e, sent2word_m, sent2word_e,
                              word_len, sub2sub, sub_len, log_D, step=global_step)
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                logger.info([x.item() for x in kls] + [grad_norm_d, grad_norm_g])
                if grad_norm_d == 0 or grad_norm_g == 0:
                    exit(-1)
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, (
        x, x_lengths, spec, spec_lengths, y, y_lengths, speakers, d_target_m, d_target_e, p_target, e_target, log_D,
        txt2sub, sub2phn_m, sub2phn_e, txt, txt_len, word2sub_m, word2sub_e, word_len, sub2sub, sub_len, names,
        sent2word_m, sent2word_e) in enumerate(eval_loader):
            rank = 0
            x, x_lengths = x.cuda(rank, non_blocking=True), x_lengths.cuda(rank, non_blocking=True)
            spec, spec_lengths = spec.cuda(rank, non_blocking=True), spec_lengths.cuda(rank, non_blocking=True)
            y, y_lengths = y.cuda(rank, non_blocking=True), y_lengths.cuda(rank, non_blocking=True)
